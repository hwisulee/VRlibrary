...
#pragma warning restore 0649
		...
		public static bool VR_Check = false;
		private static FileBrowser Instance
		{
			get
			{
				if( !m_instance )
				{
					if (VR_Check == true) // vr version
                    {
						m_instance = Instantiate(Resources.Load<GameObject>("SimpleFileBrowserCanvas_VR")).GetComponent<FileBrowser>();
						//DontDestroyOnLoad(m_instance.gameObject);
						m_instance.gameObject.SetActive(false);
					}
					else if (VR_Check == false) // pc version
                    {
