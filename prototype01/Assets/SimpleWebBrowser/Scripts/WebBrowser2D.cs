...
namespace SimpleWebBrowser
{
        ...
        IPointerUpHandler
    {
        ...
        private static WebBrowser2D m_instance = null;
        private static WebBrowser2D Instance
        {
            get
            {
                if (!m_instance)
                {
                    m_instance = Instantiate(Resources.Load<GameObject>("SimpleWebBrowserCanvas")).GetComponent<WebBrowser2D>();
 ...
 public void RunJavaScript(string js)
        ...
        {
            if (SimpleFileBrowser.FileBrowser.VR_Check == true) // vr version
            {
                return new Vector2(0, 0);
            }
            else if (SimpleFileBrowser.FileBrowser.VR_Check == false) // pc version
            {
