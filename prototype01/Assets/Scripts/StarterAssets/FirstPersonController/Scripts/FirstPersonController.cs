...
namespace StarterAssets
{
	...
	public class FirstPersonController : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			_controllerCheck = GetComponent<ControllerCheck>();
		...
		private void Start()
		{
			...
			if (ControllerCheck.controllercheck == false)
            {
				isInteracting = true;
				StarterAssetsInputs.SetCursorState(false);
				_controllerCheck.OnLoad();
			}
		}
		...
		{
			// code that limits behavior when in Interact.
        ...
        {
			if (_input.move != Vector2.zero && _input.sprint == false)
            {
				if (_input.jump == true)
				{
			...
			else if (_input.move != Vector2.zero && _input.sprint == true)
            {
				if (_input.jump == true)
                {
			...
			else if (_input.move == Vector2.zero && _input.jump == true)
			{
				animator.SetBool("IsJump", true);
			}
		...
		public void OnTriggerStay(Collider other)
		{
			...
			else if (other.gameObject.name == "GramophoneWithSpeaker")
            {
				if (_input.action == true)
				{
